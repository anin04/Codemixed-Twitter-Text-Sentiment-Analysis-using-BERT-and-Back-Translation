...
batch_size = 32      # Batch size untuk dataloader

# Load data dari CSV
train_df = pd.read_csv("Training_Data/BT_GoogleAPI.csv")
val_df = pd.read_csv("Validation_Data/BT_GoogleAPI.csv")
...
for sent in train_df['backtranslated_tweet']:
...
for sent in val_df['backtranslated_tweet']:
...
for epoch_i in range(epochs):
    ...
    if best_valid_loss > avg_val_loss:
        ...
        save_checkpoint("Hasil/Model/model_GoogleAPI.pt", model, best_valid_loss)
    
    # Simpan data training ke CSV setiap selesai 1 epoch
    save_training_data_to_csv("Hasil/Metrics/metrics_GoogleAPI.csv", training_stats)
